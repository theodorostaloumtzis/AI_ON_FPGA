#################
#    HLS4ML (patched for Vitis HLS ≥ 2024.1)
#################

# -----------------------------------------------------------------------------
#  This is a *drop-in* replacement for the original build_prj.tcl.
#  Changes w.r.t. upstream script:
#    • swap deprecated `config_array_partition -maximum_size` for the new
#      `-complete_threshold` option (UG1399, 2024.1+) and keep it inside `catch`.
#    • normalise European decimal commas in `clock_uncertainty` ("12,5%" → "12.5%")
        ...
        if {[string equal "$line" "log_wave -r /"]} {
            set line {source "../../../../project.tcl"
                ...
                foreach scope $scopes {
                    current_scope $scope
                    if {[catch [get_objects usedw]] == 0} { puts "$scope skipped"; continue }
...
proc compare_files {file_1 file_2} {
    if {!([file exists $file_1] && [file exists $file_2])} { return 0 }
    if {[file size $file_1] != [file size $file_2]} { return 0 }
    set fh_1 [open $file_1 r]; set fh_2 [open $file_2 r]
    set equal [string equal [read $fh_1] [read $fh_2]]
    close $fh_1; close $fh_2; return $equal
}

# ─────────────────────────────────────────────────────────────────────────────
#  Build flow starts here
# ─────────────────────────────────────────────────────────────────────────────
...
# ----------------------------- ARRAY-PARTITION -----------------------------
# Deprecated flag ‘-maximum_size’ was removed in Vitis HLS 2022.1.
if {[info exists maximum_size] && $maximum_size ne ""} {
    catch { config_array_partition -complete_threshold $maximum_size }
}
...
# Normalise decimal separator for clock-uncertainty, then apply
set clock_uncertainty [string map {"," "."} $clock_uncertainty]
if {![regexp {%$} $clock_uncertainty]} { append clock_uncertainty % }
...
if {$opt(csim)} {
    ...
    csim_design
    report_time "C SIMULATION" $time_start [clock clicks -milliseconds]
}
...
if {$opt(cosim)} {
    ...
    if {$rpt_file ne ""} {
        ...
        puts "WARNING: cosim report not found in expected locations"
    }
    report_time "C/RTL SIMULATION" $time_start [clock clicks -milliseconds]
}
...
if {$opt(vsynth)} {
    ...
    if {[file exist ${project_name}_prj/solution1/syn/verilog]} {
        ...
        puts "ERROR: Cannot find generated Verilog files. Did you run C synthesis?"; exit 1
    }
}
